...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    if ((axes == [0,2,3]) or (axes == [0,2])) and fused==True:
        ...
        def _fused_batch_norm_inference():
            ...
            mean = ((old_div(1.,batch_size))*mean) + ((old_div((batch_size-1.),batch_size))*moving_mean)[None,:,None,None]
            var = ((old_div(1.,batch_size))*var) + ((old_div((batch_size-1.),batch_size))*moving_variance)[None,:,None,None]
        ...
        if is_training is None:
            ...
            if update_moving_stats:
                ...
                def _force_updates():
                    ...
                    update_moving_mean = tf.assign(moving_mean, ((old_div(float_stats_iter,(float_stats_iter+1)))*moving_mean) + ((old_div(1,(float_stats_iter+1)))*batch_mean))
                    update_moving_variance = tf.assign(moving_variance, ((old_div(float_stats_iter,(float_stats_iter+1)))*moving_variance) + ((old_div(1,(float_stats_iter+1)))*batch_var))
