...
def Deconv2D(
    ...
    with tf.name_scope(name) as scope:
        ...
        if he_init:
            filters_stdev = np.sqrt(old_div(4.,(fan_in+fan_out)))
        else: # Normalized init (Glorot & Bengio)
            filters_stdev = np.sqrt(old_div(2.,(fan_in+fan_out)))
        ...
        if weightnorm:
            ...
            with tf.name_scope('weightnorm') as scope:
                ...
                filters = filters * tf.expand_dims(old_div(target_norms, norms), 1)
